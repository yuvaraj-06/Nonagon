...
class FireAlertCard extends Component {
  constructor(props) {
    super(props);
    this.state = {
      exampleModal: false,
      fire_detection_true: 0,
  ...
  fetchData = (nextProps) => {
    this._ismounted &&
      axios
        .get(
          nodeBaseURL +
            `fire_detection/aggregate/${nextProps.outlet}/${nextProps.time}?fromdate=${nextProps.startDate}&tilldate=${nextProps.endDate}`,
          {
            headers: {
        ...
        .then((res) => {
          this._ismounted &&
            this.setState({
              fire_detection_true: res.data.fire_detection_true,
                ...
                <Row>
                  <Col md="12">
                    <span className="h2 font-weight-bold mb-0pl-3">
                      ...
                      {this.state.fire_detection_true}
                    </span>
                  </Col>
                </Row>
              </div>
