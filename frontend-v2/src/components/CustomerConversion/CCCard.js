...
class CustomerConversionCard extends Component {
  constructor(props) {
    super(props);
    this.state = {
      exampleModal: false,
  ...
  fetchData = (nextProps) => {
    this._ismounted &&
      axios
        .get(
          nodeBaseURL +
          `customer_conversion/aggregate/${nextProps.outlet}/${nextProps.time}?fromdate=${nextProps.startDate}&tilldate=${nextProps.endDate}`,
          {
            headers: {
        ...
        .then((res) => {
          this._ismounted &&
            this.setState({
              customer_conversion_rate: res.data.customer_conversion_rate.toFixed(2),
        ...
        <CardBody>
                ...
                <Row>
                  <Col md="12">
                    <span className="h2 font-weight-bold mb-0pl-3">
                      ...
                      {this.state.customer_conversion_rate} %
                    </span>
                  </Col>
                </Row>
              </div>
          ...
          <div className="mt-2 mb-0 text-sm">
            <div className="mt-1">
              ...
              <Link to='/admin/customer-conversion'>
                <Button
            ...
            {/*  */}
            <Col>
              <div className="explr-i" id="explr-tooltip-CCC">
                <i className=" fas fa-info" />
              </div>
              <Tooltip
                placement="top-start"
                isOpen={this.state.tooltipOpen}
                target="explr-tooltip-CCC"
                toggle={() => this.toggleTooltip()}
              >
                The AI monitors the customer conversion of the store.
              </Tooltip>
            </Col>
            {/* ppe, hygiene index, mask deviation */}
            <Modal
