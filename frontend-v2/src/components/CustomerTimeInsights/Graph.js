  ...
  fetchData = (nextProps) => {
    this._ismounted &&
      axios
        .get(
          nodeBaseURL +
          `avg_time_spent/list/${nextProps.outletCode}/${nextProps.time}`,
          {
            headers: {
        ...
        .then((result) => {
          ...
          var displayCounterData = getPopulatedGraphData(
            ...
            convertCTIData(result.data, "avg_display_counter_timespent"),
            this.props.time
          );
          var billingCounterData = getPopulatedGraphData(
            getGraphTimeFrame(nextProps.time),
            convertCTIData(result.data, "avg_billing_counter_timespent"),
            this.props.time
          );
          var customerWaitingData = getPopulatedGraphData(
            getGraphTimeFrame(nextProps.time),
            convertCTIData(result.data, "avg_waiting_around_timespent"),
            this.props.time
          );
  ...
  render() {
    ...
    return (
      <div>
        <Card>
          <CardHeader>
            <span className="h4">Shows the customer insight time.</span>
          </CardHeader>
