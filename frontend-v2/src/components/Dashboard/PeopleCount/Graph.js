...
class Graph extends Component {
  ...
  fetchData = (nextProps) => {
    this._ismounted && axios.get(
      nodeBaseURL +
      `customers/list/${nextProps.outlet}/${nextProps.time}?fromdate=${nextProps.startDate}&tilldate=${nextProps.endDate}`,
      {
        headers: {
      ...
      .then((result) => {
        var finalData = getPopulatedGraphData(getGraphTimeFrame(nextProps.time), result.data, this.props.time);
  ...
  render() {
    ...
    return (
      <Card>
        <CardHeader>
          <span className="h4">
            Total number of people present in the store
          </span>
        </CardHeader>
