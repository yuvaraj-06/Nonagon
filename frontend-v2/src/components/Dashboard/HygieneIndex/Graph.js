...
import getPopulatedGraphData, { convertHygieneIndexData } from "utilFunctions/getPopulatedGraphData";
...
class Graph extends Component {
  ...
  fetchData = (nextProps) => {
    this._ismounted && axios.get(
      nodeBaseURL +
      `hygieneIndex/list/${nextProps.outlet}/${nextProps.time}?fromdate=${nextProps.startDate}&tilldate=${nextProps.endDate}`,
      {
        headers: {
      ...
      .then((result) => {
        var finalData = getPopulatedGraphData(getGraphTimeFrame(nextProps.time), convertHygieneIndexData(result.data), this.props.time);
        let y = Object.values(finalData);
        let x = Object.keys(finalData).map(item => {
          return getConvertedTime(
            item,
            this.props.outletTimezone,
            this.props.time === 'day' ? hour24TimeFormat : graphDateFormat
          );
        });
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <h5 className="h3 mb-0">Hygiene Deviation Trend</h5>
        </CardHeader>
