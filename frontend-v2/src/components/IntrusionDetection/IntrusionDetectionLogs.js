...
class FireAlertLogs extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      logsData: [],
      logsColumnInfo: [
        ...
        {
          dataField: "intrusion_detection",
          text: "Intrusion Detection Status",
          sort: false,
        },
        {
          dataField: "media",
      ...
      logsName: "Intrusion Detection",
      outlet: props.outlet,
      time: props.time,
      daterange: "Loading...",
      filterParameter: "intrusion_detection",
      dynamicFilterArray: [],
  ...
  fetchData = (nextProps) => {
    this._ismounted &&
      axios
        .get(
          nodeBaseURL +
            `intrusion_detection/logs/${nextProps.outlet}/${nextProps.time}`,
          {
            headers: {
        ...
        .then((res) => {
          ...
          var x = res.data.map((r) => ({
            ...
            intrusion_detection:
              r.intrusion_detection === true ? "Detected" : "Not Detected",
            media: r.screenshot,
          }));
...
export default connect(mapStateToProps)(FireAlertLogs);
