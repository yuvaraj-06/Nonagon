...
class TVSGraph2 extends Component {
  constructor(props) {
    super(props);
    this.state = {
      timestamp: [],
  ...
  fetchData = (nextProps) => {
    this._ismounted &&
      axios
        .get(
          nodeBaseURL +
          `tvs_timespent2/list/${nextProps.outletCode}/${nextProps.time}`,
          {
            headers: {
        ...
        .then((result) => {
          ...
          var displayCounterData = getPopulatedGraphData(
            ...
            convertCTIData(result.data, "avg_lounge_timespent"),
            this.props.time
          );
          var billingCounterData = getPopulatedGraphData(
            getGraphTimeFrame(nextProps.time),
            convertCTIData(result.data, "avg_discussion_timespent"),
            this.props.time
          );
          var customerWaitingData = getPopulatedGraphData(
            getGraphTimeFrame(nextProps.time),
            convertCTIData(result.data, "avg_insurance_timespent"),
            this.props.time
          );
  ...
  render() {
    let series = [
      ...
      {
        name: "Insurance and Billing Counters",
    ...
    return (
      <div>
        <Card>
          <CardHeader>
            <span className="h4">Shows the customer insight time in different regions.</span>
          </CardHeader>
...
export default connect(mapStateToProps)(TVSGraph2);
