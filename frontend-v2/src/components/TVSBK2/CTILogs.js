...
class TVSCTILogs2 extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      logsData: [],
      logsColumnInfo: [
        ...
        },
        {
          ...
          text: "Discussion Tables",
          sort: false,
        },
        {
          dataField: "waitingAroundTime",
          text: "Insurance and Billing Counters",
          sort: false,
        },
        // {
        //     dataField: "duration",
  ...
  fetchData = (nextProps) => {
    axios
      .get(
        nodeBaseURL +
        `tvs_timespent2/list/${nextProps.outlet}/${nextProps.time}`,
        {
          headers: {
      ...
      .then((res) => {
        let x = res.data.map((el) => {
          return {
            ...
            displayCounterTime: el.avg_lounge_timespent,
            billingCounterTime: el.avg_discussion_timespent,
            waitingAroundTime: el.avg_insurance_timespent,
          };
        });
...
export default connect(mapStateToProps)(TVSCTILogs2);
